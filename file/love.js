(function(window){
    ...
    // pulse scale
    if (!this._growing) this._growing = true;
    if (!this._scale) this._scale = 0.1;

    if (this._growing) {
        this._scale += 0.02;
        if (this._scale >= heart.scale) this._growing = false;
    } else {
        this._scale -= 0.015;
        if (this._scale <= 0.9 * heart.scale) this._growing = true;
    }
    ...
    ctx.save();
    ctx.fillStyle = color;
    ctx.fillText("Birthday Queen!", point.x + 28, point.y + 20);
    ctx.restore();
        },
   ...
   Tree.prototype = {
    ...
    drawHeading: function() {
        ...
        ctx.fillText("HAPPY BIRTHDAY", this.width / 2, 80); // 80px from top
        ctx.restore();
    },

    initSeed: function() {
        var seed = this.opt.seed || {};
       var heartWidth = 100; // max width your heart animation occupies
var x = seed.x || Math.min(this.width - heartWidth, this.width / 2 + 400);
 
        var y = seed.y || this.height / 2;
        var point = new Point(x , y-100);
...
const xClear = point.x - size/2;
const yClear = point.y - size/2;
ctx.clearRect(xClear, yClear, size, size);
...
startBtn.addEventListener('click', function() {
    ...
    function animateBlooms() {
        ...
        if (bloomCount < 500) {
            ...
            tree.dropImages('https://i.postimg.cc/7Zf9RjQH/heart.png', 40);
        }
    }
    animateBlooms();
